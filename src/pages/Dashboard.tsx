    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 mt-4">Loading CloudFleet...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-[calc(100vh-4rem)] text-white">
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            <p className="text-cyan-400 text-sm font-medium mb-2">OVERVIEW</p>
...
const StatCard = ({ title, value, icon, accent }: StatCardProps) => {
  ...
  return (
    <div className="bg-[#0d1b2a] border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition">
      ...
      <p className={`text-4xl font-bold mt-5 ${style.number}`}>{value}</p>

      <p className="text-xs text-slate-600 mt-2">Current count</p>
    </div>
  );
};
